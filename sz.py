#!/usr/bin/env python3
# Thanks George :) https://github.com/tinygrad/tinygrad
import os
import sys
import token
import tokenize
...
def gen_stats(base_path="."):
    table = []
    for path, _, files in os.walk(os.path.join(base_path, "gnn")):
        for name in files:
    ...
    if unchanged:
        for file in unchanged:
            ...
            if file_stat_new[0][1] - file_stat_old[0][1] != 0 or file_stat_new[0][2] - file_stat_old[0][2] != 0:
                table.append([file_stat_new[0][0], file_stat_new[0][1], file_stat_new[0][1] -
                             file_stat_old[0][1], file_stat_new[0][2], file_stat_new[0][2] - file_stat_old[0][2]])
    return table
